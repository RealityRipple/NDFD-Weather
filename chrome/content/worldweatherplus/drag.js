...
const gWorldWeather_flavour = "worldweatherplus/statusbarpanel";        
const gWorldWeather_statusbarPanel = "worldweatherStatusPanel"; 
const gWorldWeather_statusbar = "status-bar";

const worldWeather_statObserver = {
  init: function () {
    var statusbarPanel = document.getElementById(gWorldWeather_statusbarPanel);
    ...
    if (insertbefore) {
      document.getElementById(gWorldWeather_statusbar).
      ...
      if (prev) 
        document.getElementById(gWorldWeather_statusbar).insertBefore(statusbarPanel, prev);
      else
        document.getElementById(gWorldWeather_statusbar).appendChild(statusbarPanel);
    }
  },

  getSupportedFlavours: function () {
    var flavours = new FlavourSet();
    flavours.appendFlavour(gWorldWeather_flavour); 
    return flavours;
  },
  ...
  onDragStart: function (evt,transferData,action) {
    var elme = evt.target;
    while(elme.id != gWorldWeather_statusbarPanel) {
      elme =elme.parentNode;
    }
    var txt=elme.getAttribute("id");
    transferData.data=new TransferData();
    transferData.data.addDataForFlavour(gWorldWeather_flavour,txt);
    var status = document.getElementById(gWorldWeather_statusbarPanel);
    var statusbar = document.getElementById(gWorldWeather_statusbar);
    statusbar.setAttribute("worldWeatherdrag", true);
    ...
    while (child) {
      if (child != status) {
        ...
        child.addEventListener("dragover", function(event) { nsDragAndDrop.dragOver(event,worldWeather_statObserver); }, false);
        child.addEventListener("dragdrop", function(event) { nsDragAndDrop.drop(event,worldWeather_statObserver); }, false);
      }
      child = child.nextSibling;
    }
    // this will restore status bar to previous state if user does not drop on status bar
    window.addEventListener("dragexit", function(event) { nsDragAndDrop.dragExit(event,worldWeather_statObserver); }, true);
  },
  ...
  onDragExit: function(evt, session) { 
    ...
    if (elm == document.getElementById(gWorldWeather_statusbarPanel)) this.scheduleCleanup();
  },
  ...
  onDragOver: function (evt,flavour,session) {
    ...
    if (prev) prev.removeAttribute("worldWeatherdrag");
    if (next) next.removeAttribute("worldWeatherdrag");
    ...
    if (evt.clientX < midPointCoord)
      elm.setAttribute("worldWeatherdrag", "left");
    else
      elm.setAttribute("worldWeatherdrag", "right");
  },
  ...
  scheduleCleanup: function(evt) {
    clearTimeout(window.worldWeatherDragCleanup);
    window.worldWeatherDragCleanup = setTimeout('worldWeather_statObserver.cleanUp();',700);
  },
  ...
  cleanUp: function() {
    ...
    var statusbar = document.getElementById(gWorldWeather_statusbar);
    var statusbarPanel = document.getElementById(gWorldWeather_statusbarPanel);
    statusbar.removeAttribute("worldWeatherdrag");
    ...
    while (child) {
      if (child != statusbarPanel) {
        child.removeAttribute("worldWeatherdrag");
        child.removeEventListener("dragover", function(event) { nsDragAndDrop.dragOver(event,worldWeather_statObserver); }, false);
        child.removeEventListener("dragdrop", function(event) { nsDragAndDrop.drop(event,worldWeather_statObserver); }, false);
      }
      child = child.nextSibling;
    }
    window.removeEventListener("dragexit", function(event) { nsDragAndDrop.dragExit(event,worldWeather_statObserver); }, true);
  }
};

// its go time
window.addEventListener('load', worldWeather_statObserver.init, false); 